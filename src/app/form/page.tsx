...
const FillFeedbackForm = () => {
  ...
  const getFeedbacksData = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        setFeedbackUser(docSnap?.data()?.review[0]);

        const filterUserArr = docSnap?.data()?.review?.map((it: any) => it.id);
        ...
        // const filterTeamsArr = docSnap?.data()?.review?.map((it: any) => it.teamEmail);
        // const teamsArr = filterTeamsArr?.filter((it: string) => it !== undefined);
        ...
        console.log("filteredUsersFromTeams", filteredUsersFromTeams);
        setPeoplesToReviewArr(filteredUsersFromTeams);

        console.log("teamUsersArr", teamUsersArr);
        console.log("teamUsersArr", ...teamUsersArr);
        // const finalArr =
        // const contains = dd?.teamUsers?.filter((it:any)=>usersArr.includes(it.id))
  ...
  console.log("formQueDetails", formQueDetails);
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      console.log("payloadMTE", payloadMTE);

      const userId = doc(db, "feedback_form", feedbackId);
      await updateDoc(
        userId,
        formQueDetails?.feedback_type === MTE ? payloadMTE : payload
      );
      if (formQueDetails?.feedback_type !== MTE) {
        ...
        >
          <Box>
            ...
            <Select
              ...
              value={feedbackUser}
              displayEmpty
              // defaultValue={formQueDetails?.review?.length && formQueDetails?.review[0]}
              input={<OutlinedInput />}
              renderValue={(selected: any) => {
                console.log("selected", selected);
                // if (formQueDetails?.review?.length) {
                //   formQueDetails?.review[0].firstName +
                //     " " +
                //     formQueDetails?.review[0].lastName;
                // }
